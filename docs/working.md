...
- 修复 Workbench 两处状态表达：run detail 轮询现在同步更新 Recent Runs 的 status/session mapping，不再让左栏停在创建瞬间的 `queued`；右栏拆分 Workspace Files 与 Checkpointed Artifacts，前者从 normalized `file.written` event 展示 Sandbox 已写路径及 checkpoint 状态，后者只展示已持久化到 Turso、可预览下载的版本。
